...
const emptyForm = { title: "", slug: "", content: "", excerpt: "", tags: "", featuredImage: "", videoEmbed: "", category: "", isTopStory: false };
type Blog = { _id: string; title: string; slug: string; content: string; excerpt: string; publishedAt: string; tags: string[]; featuredImage?: string; videoEmbed?: string; audioUrl?: string; category?: string; isTopStory?: boolean; readCount?: number };
...
export default function AdminBlogPage() {
  ...
  function edit(item: Blog) {
    ...
    const next = { title: item.title, slug: item.slug, content: item.content || "", excerpt: item.excerpt || "", tags: item.tags?.join(", ") || "", featuredImage: item.featuredImage || "", videoEmbed: item.videoEmbed || "", category: item.category || "", isTopStory: item.isTopStory ?? false };
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (!form.title.trim() || !form.content.trim() || !slug) {
      setValidationError("Add a title, article content, and a URL slug before publishing."); setPreview(false); return;
    }
  ...
  const textField = (key: Exclude<keyof typeof emptyForm, "isTopStory">, label: string, hint?: string) => <label className="admin-field"><span>{label}</span><input value={form[key]} onChange={event => setForm({ ...form, [key]: event.target.value })} />{hint && <small>{hint}</small>}</label>;
  return <section>
    ...
    <form ref={editor} onSubmit={submit} onChangeCapture={collection.clearNotice} className="admin-form">
      ...
      <fieldset disabled={collection.pending} style={{ border: 0, padding: 0, minWidth: 0 }}>
        ...
        <div className="admin-actions admin-editor-save"><button type="submit" className="admin-button">{collection.pending ? generatingId ? "Generating audio…" : "Saving…" : editing ? "Save changes" : "Publish article"}</button>{(editing || dirty) && <button type="button" className="admin-button-secondary" onClick={() => { if (!dirty || confirm("Discard unsaved changes?")) reset(); }}>Cancel</button>}</div>
      </fieldset>
    </form>
    <div className="admin-list-heading"><h2>Published articles {!collection.loading && `(${collection.items.length})`}</h2><button className="admin-text-button" onClick={collection.refresh} disabled={collection.loading || collection.pending}>{collection.loading ? "Loading…" : "Refresh list"}</button></div>
    ...
    <div className="admin-list admin-blog-list" aria-busy={collection.loading}>{collection.items.map(item => <article className="admin-record" key={item._id}>
      <div className="admin-record-copy"><h3>{item.title}</h3><p>{item.slug} · {new Date(item.publishedAt).toLocaleDateString()}{item.category && ` · ${item.category}`} · {item.readCount ?? 0} views{item.isTopStory && " · Top story"}</p>{item.audioUrl && <a className="admin-text-button" href={item.audioUrl} target="_blank" rel="noopener noreferrer">Listen to audio ↗</a>}</div>
      <div className="admin-actions"><Link className="admin-text-button" href={`/blogs/${item.slug}`} target="_blank" rel="noopener noreferrer">View</Link><button className="admin-text-button" disabled={collection.pending} onClick={() => generateAudio(item)}>{generatingId === item._id ? "Generating…" : item.audioUrl ? "Regenerate audio" : "Generate audio"}</button><button className="admin-text-button" disabled={collection.pending} onClick={() => edit(item)}>Edit</button><button className="admin-danger" disabled={collection.pending} onClick={async () => { if (await collection.remove(item, item.title)) { if (editing?._id === item._id) reset(); } }}>Delete</button></div>
    </article>)}</div>
  </section>;
}
