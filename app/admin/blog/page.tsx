...
export default function AdminBlogPage() {
  ...
  const [form, setForm] = useState({ title: "", slug: "", content: "", excerpt: "", tags: "" });
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    const body = { ...form, slug: form.slug || slugify(form.title), tags: form.tags.split(",").map((s) => s.trim()).filter(Boolean) };
    const url = editing ? `/api/admin/blog/${editing._id}` : "/api/admin/blog";
    const method = editing ? "PUT" : "POST";
    const res = await fetch(url, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
    if (res.ok) {
      setForm({ title: "", slug: "", content: "", excerpt: "", tags: "" });
      setEditing(null);
      fetchItems();
    }
  }
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="mb-8 p-4 bg-card rounded border border-border space-y-3">
        ...
        <div className="flex gap-2">
          ...
          {editing && <button type="button" onClick={() => { setEditing(null); setForm({ title: "", slug: "", content: "", excerpt: "", tags: "" }); }} className="px-4 py-2 text-muted hover:text-foreground">Cancel</button>}
        </div>
      </form>
        ...
        {items.map((item) => (
          <div key={item._id} className="p-4 bg-card rounded border border-border flex justify-between items-start">
            <div>
              ...
              <p className="text-muted text-sm">{item.slug} · {new Date(item.publishedAt).toLocaleDateString()}</p>
            </div>
            <div className="flex gap-2">
              <Link href={`/blogs/${item.slug}`} target="_blank" className="text-muted hover:text-foreground text-sm">View</Link>
              <button onClick={() => { setEditing(item); setForm({ title: item.title, slug: item.slug, content: item.content || "", excerpt: item.excerpt || "", tags: item.tags?.join(", ") || "" }); }} className="text-accent hover:underline text-sm">Edit</button>
