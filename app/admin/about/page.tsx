...
type AboutItem = { _id: string; question: string; answer: string; order: number };
...
export default function AdminAboutPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    const url = editing ? `/api/admin/about/${editing._id}` : "/api/admin/about";
  ...
  return (
    <div>
      ...
      <p className="text-muted text-sm mb-6">Manage the Q&A sections on the About page.</p>
      <form onSubmit={handleSubmit} className="mb-8 p-4 bg-card rounded border border-border space-y-3">
        ...
        <textarea placeholder="Answer" value={form.answer} onChange={(e) => setForm({ ...form, answer: e.target.value })} className="w-full px-3 py-2 bg-card border border-border rounded text-foreground" rows={4} required />
