...
export default async function ProjectsPage(){const conn=await dbConnect();const items=conn?await Project.find().sort({order:1}).lean():[];return <><BreadcrumbListSchema items={[{name:'Home',url:'/'},{name:'Projects',url:'/projects'}]}/><div className="page-shell"><header><h1>Things I’ve built.</h1><p className="page-description">Companies, products, experiments, and tools. Some are finished. Most are still moving.</p></header>{conn?<ProjectIndex items={items.map(projectFromDocument)}/>:<p className="page-description">Projects are temporarily unavailable. Please try again shortly.</p>}</div></>;}
